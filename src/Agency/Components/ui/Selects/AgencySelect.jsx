...
export default function AgencySelect({ agencies, selectedAgency, setSelectedAgency }) {
	return (
		<Listbox value={selectedAgency} onChange={setSelectedAgency}>
			{({ open }) => (
				<>
						...
						<Listbox.Button className="relative w-44 cursor-default rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
							<span className="flex items-center">
								<span className="ml-3 text-xs  block truncate">
									{selectedAgency ? selectedAgency : "Filtrar por Agencia"}
								</span>
							</span>
